...
**Presentation index vs decode order (critical):** PyAV/Ffmpeg expose decoded pictures as they are emitted from the decoder; for typical long-GOP codecs (e.g. H.264/H.265), **decode order is not the same as presentation order** when B-frames are present. Assigning cache keys by incrementing a counter on each successive decoded packet (`decode_index += 1`) **does not** reliably yield the stream’s **presentation** frame index for each raster. Wrong `(index → pixels)` mapping causes frames to appear out of order or “wrong frame under the right number,” and failures can **persist** if the cache refuses to overwrite an existing entry for that key (see § Frame Cache — cache key correctness). The decode engine must tie each decoded picture to a **presentation** identity derived from timestamps (e.g. PTS/`best_effort_timestamp` in stream `time_base`) and map through elapsed presentation **seconds** to **0-based UI frame indices** using metadata `fps` (CFR baseline). Containers whose earliest encoded picture is not at stream timeline zero still align with UI frame 0 by subtracting a **presentation floor**: the minimum PTS-derived presentation time observed across demuxed video packets for that stream (demux-only probe once per open), then `frame_index ≔ int((t − t_min) · fps)` with truncation/clamping.

**Important follow-on requirement:** timestamp-based indexing alone is **not sufficient** if decoded outputs are consumed in emission order without presentation reordering guarantees. The playback path must enforce monotonic presentation progression for requested/displayed frames (or equivalent reorder-buffer semantics), so “first decoded frame that maps to index N” cannot regress against the previously displayed presentation instant. This must be validated with real reordering-heavy samples where naive timestamp-to-index matching can still produce visible forward/back jitter.

**Timeline/FPS mental model (scope):** A single metadata `fps` is adequate for **shared timeline ↔ integer frame index** in many CFR workflows, but it is not a substitute for **per-frame PTS** when labeling decoded pictures. Variable frame rate (VFR) streams need timestamp-driven logic end-to-end.
...
- **Cache key correctness:** Cached bitmaps must be stored **only** under the **presentation frame index** they actually represent (see § Decode Engine — Presentation index vs decode order). A mis-labeled insert must not remain authoritative: if the insert path skips updates when a key already exists, a single mis-assignment can stick and corrupt playback until invalidation. Policy must explicitly allow **replacing** or **invalidating** an entry when a newer decode proves the stored raster belongs to a different index or when correcting indexing logic.
